...
export default function ProfileTab({ userName, userEmail, onLogout }: Props) {
  ...
  return (
    <div className="flex flex-col h-full no-scrollbar overflow-y-auto" style={{ background: '#07090f' }}>
      ...
      {/* Header */}
      <div className="pt-14 pb-6 flex flex-col items-center relative z-10">
        {/* Avatar */}
        <div className="relative mb-4">
          ...
          <div className="absolute bottom-1 right-1 w-4 h-4 rounded-full border-2"
            style={{ background: '#22c55e', borderColor: '#07090f' }} />
        </div>

        <h2 className="text-white font-bold text-xl">{userName || 'User'}</h2>
        <p className="text-sm mt-0.5" style={{ color: 'rgba(147,197,253,0.55)' }}>{userEmail}</p>
        ...
        <div className="mt-3 px-3 py-1 rounded-full flex items-center gap-1.5"
          ...
          <span className="text-xs font-medium" style={{ color: 'rgba(147,197,253,0.8)' }}>UiPath Account</span>
        </div>
      </div>

      {/* Settings sections */}
      <div className="px-4 flex flex-col gap-5 relative z-10 pb-6">
        {SETTINGS.map((section) => (
          <div key={section.section}>
            <p className="text-xs font-semibold tracking-widest mb-2 px-1"
            ...
            <div className="rounded-2xl overflow-hidden"
              ...
              {section.items.map((item, idx) => (
                <button key={item.label} className="w-full flex items-center gap-3 px-4 py-3.5 transition-colors active:bg-white/10"
                  style={{ borderBottom: idx < section.items.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                  ...
                  <span className="flex-1 text-sm text-left" style={{ color: 'rgba(255,255,255,0.85)' }}>{item.label}</span>
                  {item.value ? (
                    <span className="text-xs" style={{ color: 'rgba(147,197,253,0.45)' }}>{item.value}</span>
                  ) : (
                    <svg className="w-4 h-4" style={{ color: 'rgba(255,255,255,0.2)' }} fill="none" viewBox="0 0 24 24" stroke="currentColor">
        ...
        <button
          onClick={onLogout}
          className="w-full flex items-center gap-3 px-4 py-4 rounded-2xl transition-colors active:opacity-80"
        ...
        >
          ...
          <span className="flex-1 text-sm font-medium text-left" style={{ color: '#f87171' }}>Sign Out</span>
        </button>
      </div>
    </div>
  );
